...
class CppChecker(BaseChecker):
    ...
    def detect(self) -> bool:
        ...
        return (self.context.project_root / "CMakeLists.txt").exists() or (
            self.context.project_root / "Makefile"
        ).exists()
    ...
    def run_formatter(self) -> CommandResult:
        ...
        if targets == ["."]:
            ...
            for ext in ("*.c", "*.cpp", "*.h", "*.hpp", "*.cc", "*.cxx"):
                targets.extend(
                    [
                        str(p.relative_to(self.context.project_root))
                        for p in self.context.project_root.rglob(ext)
                    ]
                )
            if not targets:
                return CommandResult(
        ...
        cmd = self.docker_wrap(
            ["clang-format", "--dry-run", "-Werror"] + targets, "silkeh/clang:latest"
        )
